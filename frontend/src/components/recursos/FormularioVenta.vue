...
<script>
import { getVenta, agregarVenta, editarVenta } from '../../services/recursos/ventas.service';
import { getEmpleados, getEmpleado } from "../../services/recursos/empleados.service";
import { getClientes, getCliente } from "../../services/recursos/clientes.service";
  ...
  methods: {
    submit() {
      if(this.validarDatos()) {
        ...
        agregarVenta(venta)
            .then(res => {
                this.mostrarMensaje('Venta registrada', res.data.message, 'success', 2000);
    ...
    editar() {
      if(this.validarDatos()) {
        ...
        editarVenta(venta)
          .then(res => {
            this.mostrarMensaje('Venta editada', res.data.message, 'success', 2000);
      ...
      if(this.ventaEditar) {
        getVenta(this.ventaEditar)
          .then(response => {
            ...
            this.fechaVenta = response.data[0].fechaVenta.slice(0, 19).replace('T', ' ');
